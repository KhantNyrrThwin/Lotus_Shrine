...
function Meditation() {
  ...
  const poseDetectionLoop = async () => {
    if (!webcamRef.current || !canvasRef.current || !ctxRef.current || !modelRef.current) return;
  ...
  const startPoseDetection = async () => {
    try {
      ...
      // Ensure canvas context is setup only once
      if (canvasRef.current && !ctxRef.current && webcamRef.current) {
