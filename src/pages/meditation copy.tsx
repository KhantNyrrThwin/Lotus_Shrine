...
function Meditation() {
  ...
  const poseDetectionLoop = async () => {
    ...
    try {
      // Update webcam frame and draw it to canvas
      ...
      // Save the current canvas state before drawing pose
      ctxRef.current.save();
      ...
      if (modelRef.current) {
        ...
        if (pose && pose.keypoints && Array.isArray(pose.keypoints) && pose.keypoints.length > 0) {
          try {
            ...
            if (ctxRef.current) {
              // Reset canvas drawing styles
              ctxRef.current.lineWidth = 2;
              ctxRef.current.strokeStyle = '#00ff00';
              ctxRef.current.fillStyle = '#ff0000';
              ...
              // Test drawing a simple circle to verify canvas is working
              ctxRef.current.beginPath();
              ctxRef.current.arc(50, 50, 10, 0, 2 * Math.PI);
              ctxRef.current.fillStyle = '#ff0000';
              ctxRef.current.fill();
