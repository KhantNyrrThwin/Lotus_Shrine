...
const RequestEmail = () => {
  ...
  useEffect(() => {
    ...
    if (length < 8) {
      ...
      message =
        "အားနည်းနေပါသေးသည် - အင်္ဂလက္ခရာကြီး၊ အက္ခရာသေး၊ ဂဏန်းနှင့် သင်္ကေတများ ပေါင်းစပ်ထည့်သွင်းပါ";
      strength = "weak";
    } else {
        ...
        <div className="flex bg-[url('./assets/forgot_password.png')] w-full h-screen items-center justify-center 2xl:h-[800px]">
          <div className="w-[489px] h-[446px] bg-[#E2E2E2B3] rounded-2xl flex flex-col justify-center">
            <h2 className="ml-5 mt-5 mb-10 text-[20px] font-extrabold text-[#40320D]">
              အကောင့်ဖွင့်ထားသည့် Email အား ရိုက်ပါ
            </h2>
            <form
            ...
            >
              ...
              {password && !errors.password && (
                <p
                  className={`text-sm -mt-2 self-start ml-11 ${getStrengthColor()}`}
                >
                  {strengthMessage}
                </p>
              )}
              ...
              <a
                ...
                className="mt-2 hover:underline cursor-pointer"
              >
                For Test onlt Code ရိုက်ဖို့ Page အတွက်ပါ ကိုပိုင် (ပြီးရင်
                Delete လုပ်ပါ)
              </a>
            </form>
          </div>
